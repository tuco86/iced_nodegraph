...
mod tests {
    ...
    fn test_pin_detection_at_zoom_1() {
        ...
        let distance = ((cursor_world.x - pin_world.x).powi(2) + 
                       (cursor_world.y - pin_world.y).powi(2)).sqrt();
        
        assert!(
            distance < 5.0,
            "At zoom 1.0: pin at world {:?} should be clickable at screen {:?}, but cursor_world={:?}, distance={:.2}",
            pin_world, cursor_screen, cursor_world, distance
        );
    }
    ...
    fn test_edge_click_detection_at_zoom_1() {
        ...
        let cursor_world: Point = camera.screen_to_world().transform_point(cursor_screen).into_iced();
        ...
        assert!(
            ...
            from_world, to_world, cursor_screen, cursor_world, dist
        );
    }
