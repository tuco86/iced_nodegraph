...
pub struct Pin {
    pub position: glam::Vec2,
    pub side: u32,
    pub radius: f32, // Pre-computed: includes animation scale for valid targets
    pub color: glam::Vec4,
    pub border_color: glam::Vec4,
    pub direction: u32,
    pub shape: u32, // 0=Circle, 1=Square, 2=Diamond, 3=Triangle
    pub border_width: f32,
    pub _pad0: u32, // Padding for 64-byte alignment
}
...
mod tests {
    ...
    #[test]
    fn test_node_glow_radius_at_correct_offsets() {
        let nodes = vec![
            create_test_node(0),
            create_test_node(1),
            create_test_node(2),
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            let glow_radius_bytes = &bytes[offset..offset + 4];
            let glow_radius = f32::from_le_bytes(glow_radius_bytes.try_into().unwrap());
            assert_eq!(
                glow_radius, node.glow_radius,
                "Node {} glow_radius mismatch at offset {}: expected {}, got {}",
                i, offset, node.glow_radius, glow_radius
            );
        }
    }
