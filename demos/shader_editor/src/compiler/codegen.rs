...
impl CodeGenerator {
    pub fn generate_node_function(
        ...
        match node_type {
            ...
            ShaderNodeType::UV => {
                format!("fn {}() -> vec2<f32> {{ return in.world_uv; }}\n", func_name)
            }
            ShaderNodeType::Time => {
                format!("fn {}() -> f32 {{ return uniforms.time; }}\n", func_name)
            }
            ShaderNodeType::MousePos => {
                format!("fn {}() -> vec2<f32> {{ return uniforms.cursor_position; }}\n", func_name)
            }
            ShaderNodeType::Resolution => {
                format!("fn {}() -> vec2<f32> {{ return uniforms.viewport_size; }}\n", func_name)
            }
            ShaderNodeType::CameraZoom => {
                format!("fn {}() -> f32 {{ return uniforms.camera_zoom; }}\n", func_name)
            }
            ShaderNodeType::CameraPosition => {
                format!("fn {}() -> vec2<f32> {{ return uniforms.camera_position; }}\n", func_name)
            }
            ...
            ShaderNodeType::VecCombine4 => {
                ...
                format!("fn {}() -> vec4<f32> {{ return vec4({}, {}, {}, {}); }}\n", func_name, x, y, z, w)
            }
            ...
            ShaderNodeType::SDF_Intersection => Self::gen_binary_func(graph, node, "opIntersection"),
            ShaderNodeType::SDF_SmoothUnion => Self::gen_ternary_func(graph, node, "opSmoothUnion"),
            ShaderNodeType::SDF_SmoothSubtraction => Self::gen_ternary_func(graph, node, "opSmoothSubtraction"),
            ShaderNodeType::SDF_SmoothIntersection => Self::gen_ternary_func(graph, node, "opSmoothIntersection"),

            // Output nodes don't generate functions
            ShaderNodeType::OutputEdge | ShaderNodeType::OutputBackground
            | ShaderNodeType::OutputNode | ShaderNodeType::OutputPin
            | ShaderNodeType::OutputFinal => String::new(),

            // Default
            _ => {
                format!("fn {}() -> f32 {{ return 0.0; }} // TODO: Implement {}\n",
                    func_name, node_type.name())
            }
        }
    }
