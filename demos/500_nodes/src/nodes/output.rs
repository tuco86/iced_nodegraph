...
where
    Message: Clone + 'a,
{
    let pins = column![pin!(Left, "col", Input, "vec4", Color::from_rgb(0.9, 0.5, 0.9)),].spacing(1);

    column![title_bar("Base Color", theme), container(pins).padding([4, 0])]
        .width(140.0)
        .into()
}
...
where
    Message: Clone + 'a,
{
    let pins = column![pin!(Left, "val", Input, "float", Color::from_rgb(0.9, 0.9, 0.9)),].spacing(1);

    column![title_bar("Roughness", theme), container(pins).padding([4, 0])]
        .width(140.0)
        .into()
}
    ...
    Message: Clone + 'a,
{
    ...
    column![title_bar("Metallic", theme), container(pins).padding([4, 0])]
        .width(140.0)
        .into()
}
...
where
    Message: Clone + 'a,
{
    let pins = column![pin!(Left, "col", Input, "vec4", Color::from_rgb(0.9, 0.9, 0.3)),].spacing(1);

    column![title_bar("Emission", theme), container(pins).padding([4, 0])]
        .width(140.0)
        .into()
}
...
where
    Message: Clone + 'a,
{
    let pins = column![pin!(Left, "N", Input, "vec3", Color::from_rgb(0.5, 0.7, 0.9)),].spacing(1);
